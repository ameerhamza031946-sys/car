...
makes = st.sidebar.multiselect('Select Makes', options=df['make'].unique(), default=df['make'].unique()[:5] if len(df['make'].unique()) > 0 else [])
years = st.sidebar.slider('Select Year Range', int(df['year'].min()), int(df['year'].max()), (int(df['year'].min()), int(df['year'].max())))
bodies = st.sidebar.multiselect('Select Body Types', options=df['body'].unique(), default=df['body'].unique()[:3] if len(df['body'].unique()) > 0 else [])
...
if filtered_df.empty:
    ...
    with col2:
        ...
        if not avg_price.empty:
            ...
            st.write("No data available for average price by make.")
